...
runDir = sprintf('gpfa/mat_results/run%03d', runIdx);
if isdir(runDir)
...
% Fail gracefully to parallelize=false if Matlab version is older than 
% 8.2 (R2013b).
if (verLessThan('matlab', '8.2'))
    fprintf(['Parallelization is only supported for Matlab versions ',...
        'newer than 2013b.\nProceeding without parallelization...']);
    parallelize = false;
end

% Fail gracefully to parallelize=false if Parallel Computing Toolbox cannot
% be detected in the current Matlab installation.
try 
   tmp = ver('distcomp');
catch
    fprintf(['Cannot detect Parallel Computing Toolbox. Proceeding ',...
        'without parallelization.\nPlease install toolbox to avail ', ...
        'speedup from parallelization.\nProceeding without parallelization.']);
    parallelize = false;
end

if (parallelize)
    % Check that parallelize is true and a license exists for the
    % Parallel Computing Toolbox.   
    poolobj=gcp('nocreate');
    if ((length(cvf_params) > 1) && isempty(poolobj))
